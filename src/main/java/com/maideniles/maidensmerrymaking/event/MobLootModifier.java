...
public class MobLootModifier extends LootModifier {
    ...
    protected MobLootModifier(ILootCondition[] conditionsIn, Item addition) {
        super(conditionsIn);
    ...
    protected List<ItemStack> doApply(List<ItemStack> generatedLoot, LootContext context) {
        ...
        if(context.getRandom().nextInt(20) ==0) {
            generatedLoot.add(new ItemStack(addition, MerryMakingConfig.COLORED_EGG_SPAWN.get()));
        }
        return generatedLoot;
    }
    @SubscribeEvent
    public static void registerModifiers(RegistryEvent.Register<GlobalLootModifierSerializer<?>> registryEvent) {
        registryEvent.getRegistry().register(new Serializer().setRegistryName(MaidensMerryMaking.MOD_ID, "mob_loot"));
    }

    public static class Serializer extends GlobalLootModifierSerializer<MobLootModifier> {

        @Override
        public MobLootModifier read(ResourceLocation name, JsonObject object, ILootCondition[] conditionsIn) {
            Item addition = ForgeRegistries.ITEMS.getValue(
                    new ResourceLocation(JSONUtils.getString(object, "addition")));
            return new MobLootModifier(conditionsIn, addition);
        }
