...
public class ConnectingTwinklingIcicleWhite extends HorizontalDecoBlock {
    ...
    private VoxelShape getShape(BlockState state)
    {
        return SHAPES.computeIfAbsent(state, state1 ->
        {
            ...
            ConnectingTwinklingIcicleWhite.Type type = state1.get(TYPE);
    ...
    private BlockState getDecorationState(BlockState state, IWorld world, BlockPos pos)
    {
        ...
        if(backState.getBlock() instanceof ConnectingTwinklingIcicleWhite)
        {
            if(backState.get(DIRECTION) == direction.rotateY())
            {
                ...
                if(!(leftState.getBlock() instanceof ConnectingTwinklingIcicleWhite) || leftState.get(DIRECTION) == direction.getOpposite())
                {
            ...
            if(backState.get(DIRECTION) == direction.rotateYCCW())
            {
                ...
                if(!(rightState.getBlock() instanceof ConnectingTwinklingIcicleWhite) || rightState.get(DIRECTION) == direction.getOpposite())
                {
