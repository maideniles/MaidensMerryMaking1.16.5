...
{
    public static final EnumProperty<Type> TYPE = EnumProperty.create("type", ConnectingGarland.Type.class);
    ...
    private VoxelShape getShape(BlockState state)
    {
        return SHAPES.computeIfAbsent(state, state1 ->
        {
            ...
            ConnectingGarland.Type type = state1.get(TYPE);
    ...
    private BlockState getDecorationState(BlockState state, World world, BlockPos pos)
    {
        ...
        if(frontState.getBlock() instanceof ConnectingGarland)
        {
            ...
            {
                return state.with(TYPE, ConnectingGarland.Type.RIGHT_CORNER);
            }
            else if(frontState.get(DIRECTION) == direction.rotateYCCW())
            {
                return state.with(TYPE, ConnectingGarland.Type.LEFT_CORNER);
            }
        }
        ...
        if(backState.getBlock() instanceof ConnectingGarland)
        {
            if(backState.get(DIRECTION) == direction.rotateY())
            {
                ...
                if(!(leftState.getBlock() instanceof ConnectingGarland) || leftState.get(DIRECTION) == direction.getOpposite())
                {
                    return state.with(TYPE, ConnectingGarland.Type.LEFT_CORNER_INVERTED);
                }
            }
            if(backState.get(DIRECTION) == direction.rotateYCCW())
            {
                BlockState rightState = world.getBlockState(pos.offset(direction.rotateY()));
                if(!(rightState.getBlock() instanceof ConnectingGarland) || rightState.get(DIRECTION) == direction.getOpposite())
                {
                    return state.with(TYPE, ConnectingGarland.Type.RIGHT_CORNER_INVERTED);
                }
            }
        }

        return state.with(TYPE, ConnectingGarland.Type.DEFAULT);
    }
