...
public class ConnectingTwinklingIcicleMulti extends HorizontalDecoBlock {
    ...
    private VoxelShape getShape(BlockState state)
    {
        return SHAPES.computeIfAbsent(state, state1 ->
        {
            ...
            ConnectingTwinklingIcicleMulti.Type type = state1.get(TYPE);
    ...
    private BlockState getDecorationState(BlockState state, IWorld world, BlockPos pos)
    {
        ...
        if(backState.getBlock() instanceof ConnectingTwinklingIcicleMulti)
        {
            if(backState.get(DIRECTION) == direction.rotateY())
            {
                ...
                if(!(leftState.getBlock() instanceof ConnectingTwinklingIcicleMulti) || leftState.get(DIRECTION) == direction.getOpposite())
                {
            ...
            if(backState.get(DIRECTION) == direction.rotateYCCW())
            {
                ...
                if(!(rightState.getBlock() instanceof ConnectingTwinklingIcicleMulti) || rightState.get(DIRECTION) == direction.getOpposite())
                {
